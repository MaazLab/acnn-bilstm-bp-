...
def run_overfit_test(
    ...
    y     : ndarray, shape (N_total_train, 2)
        Pass normalised labels when scaler is provided, raw mmHg otherwise.
    ...
    scaler : BPScaler, optional
        When provided, y is treated as normalised.  Predictions are
        denormalised back to mmHg for display and for the PASS/FAIL verdict.

    Returns
    -------
    passed : bool  (True if best MAE in mmHg < 5)
    """
    logger.info("-" * 66)
    logger.info("DIAGNOSTIC 2 | Overfit test: %d samples x %d epochs", n_samples, max_epochs)
    if scaler is not None:
        logger.info("  Labels are normalised. Per-sample display and verdict are in mmHg.")
    ...
    y_over = y[overfit_idx]        # (n_samples, 2)  -- may be normalised

    # Compute mmHg values for display (always shown in mmHg)
    y_mmhg = scaler.inverse_transform(y_over) if scaler is not None else y_over

    logger.info("  Overfit sample BP targets (mmHg):")
    for i in range(len(chosen_sids)):
        logger.info("    Subject %4d | SBP=%.1f | DBP=%.1f",
                    chosen_sids[i], y_mmhg[i, 0], y_mmhg[i, 1])
    logger.info("  Target range: SBP=%.1f mmHg, DBP=%.1f mmHg (higher = harder)",
                y_mmhg[:, 0].max() - y_mmhg[:, 0].min(),
                y_mmhg[:, 1].max() - y_mmhg[:, 1].min())
    ...
    # Early-stop threshold in training units (normalised or raw mmHg)
    converge_thresh = 0.05 if scaler is not None else 1.0

    log_at = {1, 10, 25, 50, 100, 200, 300, 400, 500}
    best_mae = float("inf")
    best_state = copy.deepcopy(model.state_dict())

    units = "norm" if scaler is not None else "mmHg"
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch in log_at:
            logger.info("  Overfit epoch %3d/%d | Train MAE: %.4f %s",
                        epoch, max_epochs, mae, units)

        if mae < converge_thresh:
            logger.info("  Overfit epoch %3d/%d | Train MAE: %.4f %s  [converged early]",
                        epoch, max_epochs, mae, units)
            break

    # Final prediction check with best weights
    model.load_state_dict(best_state)
    model.eval()
    with torch.no_grad():
        X_t = torch.tensor(X_over, dtype=torch.float32).to(device)
        preds_raw = model(X_t).cpu().numpy()

    # Always report in mmHg
    preds_mmhg = scaler.inverse_transform(preds_raw) if scaler is not None else preds_raw
    mae_mmhg = float(np.abs(preds_mmhg - y_mmhg).mean())

    logger.info("  Per-sample predictions (best weights, MAE=%.2f mmHg):", mae_mmhg)
    for i in range(len(chosen_sids)):
        logger.info(
            "    Sample %2d | True SBP=%.1f DBP=%.1f | Pred SBP=%.1f DBP=%.1f"
            " | Err SBP=%.1f DBP=%.1f",
            i + 1,
            y_mmhg[i, 0], y_mmhg[i, 1],
            preds_mmhg[i, 0], preds_mmhg[i, 1],
            abs(preds_mmhg[i, 0] - y_mmhg[i, 0]),
            abs(preds_mmhg[i, 1] - y_mmhg[i, 1]),
        )

    passed = mae_mmhg < 5.0
    logger.info("")
    if passed:
        logger.info("  RESULT: PASS  best MAE=%.2f mmHg < 5 mmHg", mae_mmhg)
        logger.info("    Model CAN memorise small data => pipeline is correct.")
        logger.info("    Full-training plateau is now an optimisation issue, not a code bug.")
    else:
        logger.info("  RESULT: FAIL  best MAE=%.2f mmHg >= 5 mmHg", mae_mmhg)
        if scaler is not None:
            logger.info("    Even with target normalisation the model cannot memorise 10 samples.")
            logger.info("    Investigate: model output shape, loss, CWT images, data dtype.")
        else:
